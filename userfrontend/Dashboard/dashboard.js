...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // ==================== FULL STATUS MAPS ====================
  // These cover every status your backend can set.
  const statusColors = {
  ...
  // Normalize raw backend status to uppercase underscore key
  function normalizeStatus(raw) {
    return (raw || 'PENDING').toUpperCase().replace(/-/g, '_');
  }
  ...
  async function renderDashboardPage() {
    ...
    try {
      ...
      // Stats — count every non-resolved status as active
      const stats = {
        total:    complaints.length,
        reviewed: complaints.filter(c =>
          ['IN_PROGRESS','UNDER_REVIEW','INVESTIGATING'].includes(normalizeStatus(c.status))
        ).length,
        resolved: complaints.filter(c =>
          ['RESOLVED','CLOSED','ACTION_TAKEN'].includes(normalizeStatus(c.status))
        ).length
      };
      ...
      const complaintsHtml = recentComplaints.map(complaint => `
        <tr>
          ...
          <td>
            <span class="status-badge ${statusColors[complaint.status] || 'status-pending'}">
      ...
      mainContent.innerHTML = `
        <div class="dashboard-page">
          ...
          <div class="stats-grid">
            ...
            <div class="stat-card">
              <div class="stat-icon resolved"><i class="fas fa-check-double"></i></div>
              <div class="stat-info">
    ...
    mainContent.innerHTML = `
      <div class="track-page">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;align-items:start;margin-top:24px;">
          ...
          <div id="detailPanel" style="display:none;background:#fff;border:1px solid #e5e7eb;border-radius:14px;padding:28px;position:sticky;top:24px;"></div>
        </div>
      </div>`;
        ...
        <form id="feedbackForm">
          <div class="form-group">
            ...
            <div id="selectedComplaintBox" style="
              ...
              display:${preselectedComplaint ? 'flex' : 'none'};align-items:center;gap:10px;">
            ...
            <div id="noComplaintWarning" style="
              ...
              border-radius:10px;font-size:14px;color:#9a3412;">
  ...
  function attachDashboardListeners() {
    document.querySelectorAll('.btn-view').forEach(btn => {
      btn.addEventListener('click', (e) => {
        window._trackComplaintId = e.target.dataset.id;
        navigateTo('track');
      });
    });
  ...
  function attachGrievanceListeners() {
    ...
    if (fileUpload && fileInput) {
      ...
      fileInput.addEventListener('change', (e) => {
        ...
        fileUpload.querySelector('p').textContent = fileCount > 0
    ...
    if (form) {
      form.addEventListener('submit', async (e) => {
        ...
        try {
          ...
          form.reset();
          if (fileUpload) fileUpload.querySelector('p').textContent = 'Click to upload or drag and drop';
        } catch (error) {
  ...
  async function attachTrackListeners() {
    ...
    function buildTimeline(c) {
      ...
      if (['IN_PROGRESS','UNDER_REVIEW','INVESTIGATING','ACTION_TAKEN','ESCALATED','RESOLVED','CLOSED'].includes(status)) {
        events.push({
          title:       'Under Review',
          date:        c.updated_at || c.created_at,
          description: 'Your complaint is being reviewed by our committee.'
        });
      }

      if (['INVESTIGATING'].includes(status)) {
        events.push({
          title:       'Investigating',
          date:        c.updated_at,
          description: 'A formal investigation has been initiated.'
        });
      }
      ...
      if (status === 'ACTION_TAKEN') {
        events.push({
          title:       'Action Taken',
          date:        c.updated_at,
          description: 'Appropriate action has been taken against the reported incident.'
        });
      }
      ...
      if (status === 'ESCALATED') {
        events.push({
          title:       'Escalated',
          date:        c.updated_at,
          description: 'Your complaint has been escalated to a higher authority.'
        });
      }
      ...
      if (['RESOLVED','CLOSED'].includes(status)) {
        events.push({
          title:       'Resolved',
          date:        c.updated_at,
          description: 'Your complaint has been resolved and closed.'
        });
      }
    ...
    function renderDetail(c) {
        ...
        return `
          <div style="display:flex;gap:14px;margin-bottom:${isLast ? '0' : '20px'}">
            ...
            <div style="padding-bottom:4px">
              ...
              ${event.description ? `<p style="margin:0;font-size:13px;color:#64748b">${event.description}</p>` : ''}
            </div>
          </div>`;
      }).join('');
      ...
      detailPanel.innerHTML = `
        <div style="display:flex;justify-content:space-between;align-items:flex-start;
          ...
          <span class="status-badge ${statusColors[status] || 'status-pending'}">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;
          ...
          <div>
            <p style="margin:0;font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.5px;font-weight:700">Type</p>
            <p style="margin:4px 0 0;font-size:14px;color:#1a1a2e;font-weight:500">${type}</p>
          </div>
          <div>
            <p style="margin:0;font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.5px;font-weight:700">Incident Date</p>
            <p style="margin:4px 0 0;font-size:14px;color:#1a1a2e;font-weight:500">${incDate}</p>
          </div>
          <div>
            <p style="margin:0;font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.5px;font-weight:700">Submitted On</p>
            <p style="margin:4px 0 0;font-size:14px;color:#1a1a2e;font-weight:500">${submitted}</p>
          </div>
          <div>
            <p style="margin:0;font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.5px;font-weight:700">Last Updated</p>
            <p style="margin:4px 0 0;font-size:14px;color:#1a1a2e;font-weight:500">${updated}</p>
          </div>
        </div>

        <p style="margin:0 0 6px;font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.5px;font-weight:700">Description</p>
        <p style="margin:0 0 24px;font-size:14px;color:#374151;line-height:1.7">
          ${c.description || 'No description provided.'}
        </p>

        <p style="margin:0 0 14px;font-size:10px;color:#94a3b8;text-transform:uppercase;letter-spacing:0.5px;font-weight:700">Status History</p>
        ${timelineHtml}

        ${['RESOLVED','CLOSED','ACTION_TAKEN'].includes(status) ? `
    ...
    // Auto-open if navigated from dashboard btn-view
  ...
  if (logoutBtn) {
    logoutBtn.addEventListener('click', (e) => {
      ...
      if (confirm('Are you sure you want to logout?')) {
        ['token','userId','userName','userRole'].forEach(k => localStorage.removeItem(k));
        window.location.href = '../Login/login.html';
      }
    });
  }
