...
public class EscalationService {
    ...
    public EscalationResponse escalateComplaint(Escalation escalation, Long committeeMemberId) {
        ...
        Complaint complaint = complaintRepository.findById(escalation.getComplaintId())
                .orElseThrow(() ->
                        new RuntimeException("Complaint not found with id: " + escalation.getComplaintId()));

        User committeeMember = userRepository.findById(committeeMemberId)
                .orElseThrow(() ->
                        new RuntimeException("Committee member not found with id: " + committeeMemberId));
    ...
    private EscalationResponse buildEscalationResponse(Escalation escalation) {
        ...
        User escalatedToUser = escalation.getEscalatedTo() != null
                ? userRepository.findById(escalation.getEscalatedTo()).orElse(null)
                : null;
    ...
    private EscalationResponse buildEscalationResponse(
        ...
        response.setEscalatedTo(escalation.getEscalatedTo());
        ...
        if (escalatedToUser != null) {
            response.setEscalatedToName(escalatedToUser.getName());
        }
